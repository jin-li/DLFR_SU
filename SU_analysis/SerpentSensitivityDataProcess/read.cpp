...
extern const std::string workDirectory = cmdExecuteResult("cd");
extern const std::string indexDirectory = cmdExecuteResult("cd") + "\\index";
...
{
	string readIndex[3][471];
	string matrixIndex[5][569];
	string lineRead;
	stringstream ss1, ss2;
	ifstream inFile1(_inFileName1_), inFile2(_inFileName2_);
	//cout << sizeof(covarianceMatrixStruct);
	//covarianceMatrix commaraMatrix[100][70];

	if (!inFile1) {
		cout << "Can't open file" << "\"" << _inFileName1_ << "\"!" << endl;
		return;
	}
	for (unsigned i = 0; i<471; i++) {
		getline(inFile1, lineRead);
		ss1 << lineRead;
		ss1 >> readIndex[2][i] >> readIndex[0][i] >> readIndex[1][i];
		ss1.clear();
	}
	inFile1.close();

	if (!inFile2) {
		cout << "Can't open file" << "\"" << _inFileName2_ << "\"!" << endl;
		return;
	}
	for (unsigned i = 0; i<569; i++) {
		getline(inFile2, lineRead);
		ss2 << lineRead;
		for (unsigned j = 0; j < 5; j++) ss2 >> matrixIndex[j][i];
	}
	inFile2.close();

	unsigned rowN = 0, columnN = 0;
	for (unsigned j = 0; j < 569; j++) {
		for (unsigned k = 0; k < 471; k++) {
			if (matrixIndex[1][j] == readIndex[0][k] && matrixIndex[2][j] == readIndex[1][k]) {
				rowN = k;
				break;
			}
		}
		for (unsigned k = 0; k < 471; k++) {
			if (matrixIndex[3][j] == readIndex[0][k] && matrixIndex[4][j] == readIndex[1][k]) {
				columnN = k;
				break;
			}
		}
		commaraMatrixStruct[rowN][columnN].ifExist = 1;
		commaraMatrixStruct[rowN][columnN].serialNumber = j;
		if (rowN != columnN) {
			commaraMatrixStruct[columnN][rowN].ifExist = 2;
			commaraMatrixStruct[columnN][rowN].serialNumber = j;
		}
	}
	/*
	for (unsigned i = 0; i < 471; i++) {
	for (unsigned j = 0; j < 471; j++) {
	if (i > j&&commaraMatrixStruct[i][j].ifExist) {
	commaraMatrixStruct[i][j].ifExist = 2;
	}
	}
	}
	*/
...
void sensitivityAnalysis(const string sensitivityFileName)
{
	...
	getReactionPairPosition(indexDirectory+"\\allNuclide_Reaction", indexDirectory+"\\COMMARA_2.0_index");
